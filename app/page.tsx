...
export default function HomePage() {
  ...
  const handleInstructorContact = (instructor: Instructor, method: 'phone' | 'whatsapp') => {
    if (method === 'phone') {
      ...
      const message = `Hi! I'd like to book driving lessons with ${instructor.name} in ${instructor.location}. Can you help me?`;
      const encodedMessage = encodeURIComponent(message);
                  ...
                  {filteredInstructors.map((instructor) => (
                    <InstructorCard
                      ...
                      onViewProfile={(instructor) => {
                        const message = `Hi! I'd like to view the profile and get more information about instructor ${instructor.name} in ${instructor.location}. Please provide me with detailed information about their availability and teaching style.`;
                        const encodedMessage = encodeURIComponent(message);
                        window.open(`https://wa.me/447756183484?text=${encodedMessage}`, '_blank');
                      }}
